...
static int
sb_internal_ensure_more_bytes (struct string_buffer *buffer, size_t increment)
{
...
sb_append (struct string_buffer *buffer, const char *str)
{
  ...
  if (sb_internal_ensure_more_bytes (buffer, len) < 0)
    {
      buffer->error = true;
      return -1;
    }
             ...
             va_list list)
{
  ...
  if (room < 64)
    {
      if (sb_internal_ensure_more_bytes (buffer, 64) < 0)
        {
          buffer->error = true;
          return -1;
        }
      ...
      if ((size_t) ret <= room)
      ...
      else
        {
          /* The result was truncated.  Make more room, for a second vsnprintf()
             call.  */
          if (sb_internal_ensure_more_bytes (buffer, (size_t) ret) < 0)
            {
              buffer->error = true;
              ret = -1;
            }
...
HEDLEY_MALLOC YALIBCT_ATTRIBUTE_DEALLOC_RESULT_WITH_FREE
static char *
sb_dupfree (struct string_buffer *buffer)
{
  ...
  if (sb_internal_ensure_more_bytes (buffer, 1) < 0)
    goto fail;
