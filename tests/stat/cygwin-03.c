...
setup2()
{
	int fd = open("t_file", O_RDWR|O_CREAT, MODE_RWX);				/* File handle for test file */
...
cleanup()
{
	...
	/* Remove files and temporary directory created */
        assert(remove("t_file") == 0);
        assert(remove(TEST_FILE1) == 0);
        assert(remove(DIR_TEMP) == 0);
	tst_rmdir();
