...
static void test_snprintf (void)
{
    ...
    ok (snprintf (NULL, 0, "abcd") == 4,
        "Failure to snprintf to NULL\n");
    // Usage of non-standard and non-widely supported routines removed
...
static void test_swprintf (void)
{
    ...
    ok (vswprintf_wrapper (0, NULL, 0, str_short) == 5,
        "Failure to swprintf to NULL\n");*/
    ok (swprintf (buffer, 0, str_short) == -1,
    ...
    ok (vswprintf_wrapper (0, buffer, 0, str_short) == -1,
        "Failure to swprintf to a zero length buffer\n");*/
    ok (swprintf (buffer, 0, str_empty) == -1,
                                    ...
                                    const char *format, ...)
{
    ...
    va_list valist;
    va_start(valist, format);
    ret = vfprintf(file, format, valist);
    va_end(valist);
    return ret;
}
                                     ...
                                     const wchar_t *format, ...)
{
    ...
    va_list valist;
    va_start(valist, format);
    ret = vfwprintf(file, format, valist);
    va_end(valist);
    return ret;
}
#endif
...
static void test_vsnwprintf_s(void)
{
    ...
    ok( exp == got, "length wrong, expect=%d, got=%d\n", exp, got);
    ok( !lstrcmpW(out7, buffer), "buffer wrong, got=%ls\n", (buffer));
    ...
    ok( exp == got, "length wrong, expect=%d, got=%d\n", exp, got);
    ok( !lstrcmpW(out6, buffer), "buffer wrong, got=%ls\n", (buffer));

    got = _vsnwprintf_s_wrapper(buffer, 2, _TRUNCATE, format, 123);
    ok( exp == got, "length wrong, expect=%d, got=%d\n", exp, got);
    ok( !lstrcmpW(out2, buffer), "buffer wrong, got=%ls\n", (buffer));

    got = _vsnwprintf_s_wrapper(buffer, 1, _TRUNCATE, format, 123);
    ok( exp == got, "length wrong, expect=%d, got=%d\n", exp, got);
    ok( !lstrcmpW(out1, buffer), "buffer wrong, got=%ls\n", (buffer));
}*/
...
static void test_printf_natural_string(void)
{
    const wchar_t wide[] = {'A','B','C','D',0};
    const char narrow[] = "abcd";
    const char narrow_fmt[] = "%s %s";
    const char narrow_out[] = "abcd abcd";
    const wchar_t wide_fmt[] = {'%','s',' ','%','l','s',0};
...
static void test_printf_fp(void)
{
    ...
    const struct {
        ...
        { "%a", 1, { "0x1.0000000000000p+0" }, {}, "0x1p+0"},
        { "%a", 20, { "0x1.4000000000000p+4" }, {}, "0x1.4p+4"},
        { "%a", -1, { "-0x1.0000000000000p+0" }, {}, "-0x1p+0"},
        ...
        { "%a", 4.9406564584124654e-324, { "0x0.0000000000001p-1022" }, {}, "0x1p-1074"},
...
static void test_printf_width_specification(void)
{
    ...
    /*r = snprintf(buffer, sizeof(buffer), "%0*2d", 1, 3);
    ok(r == 2, "r = %d\n", r);
    ok(!strcmp(buffer, "03"), "buffer wrong, got=%s\n", buffer);

    r = snprintf(buffer, sizeof(buffer), "%*0d", 1, 2);
    ok(r == 1, "r = %d\n", r);
    ok(!strcmp(buffer, "2"), "buffer wrong, got=%s\n", buffer);

    r = snprintf(buffer, sizeof(buffer), "% *2d", 0, 7);
    ok(r == 2, "r = %d\n", r);
