...
#include <limits>
#include <string>*/
#include "test-deps/doctest.h"
#include <stdio.h>
#include <string.h>
#include <stdarg.h>
#include <limits.h>

#if 0//NANOPRINTF_HAVE_GCC_WARNING_PRAGMAS
...
void require_equal(char const *expected, char const *fmt, ...) {
...

  {
    va_list args;
    va_start(args, fmt);
    assert(vsnprintf(buf, sizeof(buf), fmt, args) == strlen(expected));
    va_end(args);
    buf[sizeof(buf)-1] = '\0';
  }
...
#if 1//NANOPRINTF_USE_PRECISION_FORMAT_SPECIFIERS == 1
  SUBCASE("precision") {
    require_equal(                "", "%.0b", 0);
...
#if 1//NANOPRINTF_USE_FIELD_WIDTH_FORMAT_SPECIFIERS == 1
  SUBCASE("field width") {
    require_equal(   "0", "%1b", 0);
...
#if 1//NANOPRINTF_USE_LARGE_FORMAT_SPECIFIERS == 1
  SUBCASE("large") {
