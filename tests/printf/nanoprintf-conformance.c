...
#if 1//NANOPRINTF_USE_FLOAT_FORMAT_SPECIFIERS == 1
  SUBCASE("float NaN") {
      char buf[32];
      REQUIRE(snprintf(buf, sizeof(buf), "%f", (double)NAN) == 3);
      char *lowercase_nan = buf;

    // doctest can't do || inside REQUIRE
    if (strcmp(lowercase_nan, "nan") != 0) {
      REQUIRE(strcmp(lowercase_nan, "-nan") == 0);
    } else {
      REQUIRE(strcmp(lowercase_nan, "nan") == 0);
    }

      REQUIRE(snprintf(buf, sizeof(buf), "%F", (double)NAN) == 3);
      char *uppercase_nan = buf;

    // doctest can't do || inside REQUIRE
    if (strcmp(uppercase_nan, "NAN") != 0) {
      REQUIRE(strcmp(uppercase_nan, "-NAN") == 0);
    } else {
      REQUIRE(strcmp(uppercase_nan, "NAN") == 0);
    }
  }
