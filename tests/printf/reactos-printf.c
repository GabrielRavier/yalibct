...
static void test_sprintf( void )
{
    ...
    format = "%zx";
    r = p_sprintf(buffer,format,1);
    ok(!strcmp(buffer, "1"), "Problem with \"z\" interpretation\n");
    ok( r==1, "return count wrong\n");
    ...
    if (sizeof(void *) == 8)
    {
        ...
        ok(!strcmp(buffer,"0000000000000039") || !strcmp(buffer, "0x39"),"Pointer formatted incorrectly \"%s\"\n",buffer);
        ok( r==16 || r == 4, "return count wrong\n");

        // This is prety much UB and otherwise is very unreliable anyway
        /*format = "%#020p";
        r = p_sprintf(buffer,format,(void *)57);
    ...
    // Let's test UB, what could go wrong ?
    /*format = "%3h";
    r = p_sprintf(buffer, format);
    ...
    ok(!strcmp(buffer,"8.6000"), "failed '%s'\n", buffer);
    ok( r==6, "return count wrong\n");
    ...
    format = "% 014.4e";
    r = p_sprintf(buffer, format,8.6);
    ok(!strcmp(buffer," 0008.6000e+00"), "failed: '%s'\n", buffer);
    ...
    if (sizeof(void *) == 8)
    {
        ok(!strcmp(buffer,"0000000000000000"), "failed '%s'\n",buffer);
        ok( r==16, "return count wrong\n");
    }
    else
    {
...
static void test_fcvt(void)
{
    ...
    ok( 0 == strcmp(str,"00000") || 0 == strcmp(str, "000000"), "bad return '%s'\n", str);
...
static struct {
    ...
    {999999999999.9,   3,       "100","999999999999900",         13,     12,     0, "1000" },
    /* 0.0 with different precisions */
    ...
    { 0.09999999996,   2,        "10",             "10",          0,      0,     0, "100" },
    {           0.6,   1,         "6",              "6",          0,      0,     0 },
    {           0.6,   0,          "",              "1",          0,      1,     0, NULL, 1 },
    {           0.4,   0,          "",               "",          0,      0,     0, NULL, 0, 1, "0" },
    {          0.49,   0,          "",               "",          0,      0,     0, NULL, 0, 1, "0" },
    {          0.51,   0,          "",              "1",          0,      1,     0, NULL, 1 },
...
static void test_xcvt(void)
{
    ...
    for( i = 0; strcmp( test_cvt_testcases[i].expstr_e, "END"); i++){
        ...
        str = ecvt( test_cvt_testcases[i].value,
                ...
                &decpt,
                &sign);
        ok( 0 == strncmp( str, test_cvt_testcases[i].expstr_e, 15) || (test_cvt_testcases[i].expstr_e2 != NULL && !strncmp(str, test_cvt_testcases[i].expstr_e2, 15)),
            "ecvt() %d bad return, got \n'%s' expected \n'%s%s%s'\n", i, str,
            test_cvt_testcases[i].expstr_e, test_cvt_testcases[i].expstr_e2 != NULL ? "' or \n'" : "", test_cvt_testcases[i].expstr_e2 != NULL ? test_cvt_testcases[i].expstr_e2 : "");
        ok( decpt == test_cvt_testcases[i].expdecpt_e || (test_cvt_testcases[i].expdecpt_e2 != 0 && decpt == test_cvt_testcases[i].expdecpt_e2),
            "ecvt() %d decimal point wrong, got %d expected %d\n", i, decpt,
    ...
    for( i = 0; strcmp( test_cvt_testcases[i].expstr_e, "END"); i++){
        ...
        str = fcvt( test_cvt_testcases[i].value,
                ...
                &decpt,
                &sign);
        ok( 0 == strncmp( str, test_cvt_testcases[i].expstr_f, 15) || (test_cvt_testcases[i].expstr_f2 != NULL && !strncmp(str, test_cvt_testcases[i].expstr_f2, 15)),
            "fcvt() %d bad return, got \n'%s' expected \n'%s'\n", i, str,
              test_cvt_testcases[i].expstr_f);
        ok( decpt == test_cvt_testcases[i].expdecpt_f || (test_cvt_testcases[i].expdecpt_f2 != 0 && decpt == test_cvt_testcases[i].expdecpt_f2),
            "fcvt() %d decimal point wrong, got %d expected %d\n", i, decpt,
...
static void test_vsnwprintf_s(void)
{
    ...
    // These tests removed because we ain't testing MS's broken implementations of optional functions that nobody uses
    /* Enough room. *//*
    exp = wcslen(out7);

    got = _vsnwprintf_s_wrapper(buffer, 14, _TRUNCATE, format, 123);
    ok( exp == got, "length wrong, expect=%d, got=%d\n", exp, got);
    ok( !wcscmp(out7, buffer), "buffer wrong, got=%ls\n", (buffer));
    ...
    ok( exp == got, "length wrong, expect=%d, got=%d\n", exp, got);
    ok( !wcscmp(out6, buffer), "buffer wrong, got=%ls\n", (buffer));

    got = _vsnwprintf_s_wrapper(buffer, 2, _TRUNCATE, format, 123);
    ok( exp == got, "length wrong, expect=%d, got=%d\n", exp, got);
    ok( !wcscmp(out2, buffer), "buffer wrong, got=%ls\n", (buffer));

    got = _vsnwprintf_s_wrapper(buffer, 1, _TRUNCATE, format, 123);
    ok( exp == got, "length wrong, expect=%d, got=%d\n", exp, got);
    ok( !wcscmp(out1, buffer), "buffer wrong, got=%ls\n", (buffer));*/
}
