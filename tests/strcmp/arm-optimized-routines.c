...
#include "test-deps/arm-optimized-routines.h"
      ...
      int delta)
{
  ...
  // This code is commented because the alternative below is much faster
...
main ()
{
  ...
  for (int i = 0; funtab[i].name; i++)
    {
      err_count = 0;
      for (int d = 0; d < A; d += (random() % 3) + 1)
        for (int s = 0; s < A; s += (random() % 3) + 1)
	  {
	    int n;
